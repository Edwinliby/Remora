const sectionDataEn = {
  section1: {
    title: "Start, Safety & Pregnancy Decisions",
    description: [
      "If you are pregnant, might be pregnant, or just found out you are pregnant, you are not alone.",
      "People have many different feelings at the start of pregnancy.",
      "This section helps you get started. Here you’ll find what to do first, when to get help right away, and information about pregnancy options.",
      "This section also includes information about warning signs, urgent concerns, and special situations.",
      "The Resource Guide can connect you to organizations if you need testing, urgent help, or additional support.",
      "You do not have to decide everything today.",
      "Read what feels helpful right now.",
    ],
    accordion: [
      {
        title: "Getting Started & Your Options",
        subtitle: "You are not alone. Take small, clear steps.",
        intro: [
          "When pregnancy begins, many people are learning how the healthcare system works for the first time.",
          "This guide is here to help you take small, clear steps, one at a time.",
        ],
        listTitle: "Help is available especially if:",
        listItems: [
          "You do not have insurance yet",
          "Your immigration status is complicated",
          "You do not speak English well",
          "You have moved recently or may need to move again",
        ],
        closing: "You can start where you are. Support is available.",
      },
      {
        title: "If You Think You Might Be Pregnant",
        subtitle: "How to get a test and what to do next",
        intro: [
          "You can buy a pregnancy test at most stores.",
          "You can also get a pregnancy test at:",
        ],
        listItems: [
          "Two Rivers Public Health Department (free or low-cost)",
          "Local clinics (see the Resource Directory)",
        ],
        secondaryTitle: "If the test is positive:",
        secondaryList: [
          "Try to make a prenatal appointment within 1–2 weeks",
          "Start taking a prenatal vitamin (you do not need a prescription)",
          "Apply for health coverage if you need it",
        ],
        tertiaryTitle: "Applying for Health Coverage During Pregnancy",
        tertiaryLink: "#section-4-3",
        closing: [
          "To apply for health coverage, you may need a pregnancy verification letter from a health care provider.",
          "Cost-saving tip: Two Rivers Public Health Department may provide the pregnancy test and the verification letter without a clinic visit.",
        ],
      },
      {
        title: "If You Need Care Right Away",
        subtitle: "Emergency care comes first",
        intro: [
          "If you have severe pain, heavy bleeding, fever, or feel unsafe, get care right away.",
          "Go to any emergency room.",
        ],
        listTitle: "You cannot be turned away because of:",
        listItems: ["Insurance status", "Immigration status", "Ability to pay"],
      },
      {
        title: "Need Extra Help Right Now?",
        subtitle: "Extra support during pregnancy",
        listTitle: "Help is available if you are worried about:",
        listItems: [
          "Food",
          "Housing",
          "Transportation",
          "Safety",
          "Immigration concerns",
        ],
        secondaryTitle: "You do not have to handle this alone.",
        tertiaryTitle:
          "Support is available. Please see the resource guide for local resources.",
        tertiaryLink: "/resources",
      },
      {
        title: "Your pregnancy options",
        subtitle: "Learn about parenting, adoption, and pregnancy options",
        intro: [
          "People choose different paths during pregnancy.",
          "Some people:",
        ],
        listItems: [
          "Continue the pregnancy and parent",
          "Make an adoption plan",
          "Choose to end the pregnancy",
        ],
        secondaryTitle: "Adoption",
        secondaryIntro: "Adoption is a legal option. You may:",
        secondaryList: [
          "Choose open, semi-open, or closed adoption (where birth parents are fully involved, partly involved, or not involved)",
          "Choose the family for your baby",
          "Get help with some pregnancy-related expenses",
          "Change your mind until the adoption is legally finalized",
        ],
        tertiaryTitle: "Abortion",
        tertiaryIntro:
          "Nebraska law limits abortion after 12 weeks of pregnancy, with few exceptions.",
        tertiaryList: [
          "Contact Planned Parenthood for abortion-related counseling",
        ],
      },
      {
        title: "Why Early Prenatal Care Matters",
        subtitle: "How early care helps you and your baby",
        intro: "Seeing a prenatal provider early helps:",
        listItems: [
          "Protect your health",
          "Protect your baby's health",
          "Find problems early",
          "Connect you to insurance, food help, and other support",
        ],
        closing:
          "Try to start prenatal care as soon as you can.\nTake a prenatal vitamin every day if possible.",
      },
    ],
  },
  section2: {
    title: "Prenatal Care: Access & How It Works",
    description: [
      "This section explains how prenatal care works.",
      "You'll learn:",
    ],
    descriptionListItems: [
      "Where to get prenatal care",
      "What usually happens at visits",
      "What care looks like at different parts of pregnancy",
    ],
    descriptionClosing: [
      "This section also includes information about care after delivery.",
      "You’ll also find guidance on how to ask questions, get help understanding information, and speak up during care. You are part of the care team.",
      "The Resource Guide can help you find clinics and services for prenatal and after-delivery care.",
      "This section can be used as a reference throughout pregnancy and after birth.",
    ],
    accordion: [
      {
        title: "Advocating for Yourself in Healthcare",
        subtitle: "You have rights during medical care.",
        listTitle: "You have the right to:",
        listItems: [
          "Ask questions",
          "Ask for an interpreter",
          "Bring someone with you",
          "Ask for copies of your records",
          "Change providers",
        ],
        secondaryTitle: "You can say:",
        secondaryList: [
          "“Can you explain that again?”",
          "“I need an interpreter.”",
          "“Something does not feel right.”",
        ],
      },
      {
        title: "Choosing a Prenatal Care Provider",
        subtitle: "Where you'll get care and give birth",
        intro: [
          "Not all clinics provide prenatal care.",
          "Not all hospitals deliver babies.",
          "In an emergency, go to the emergency room.",
          "It helps to ask early where you will give birth.",
        ],
        listTitle: "When calling a clinic, you can ask:",
        listItems: [
          "Do you provide prenatal care?",
          "Do you deliver babies?",
          "Where would I give birth?",
          "What hospital should I go to?",
          "What if labor starts at night?",
          "Who do I call?",
          "Do you have an interpreter?",
        ],
        secondaryTitle: "In Rural Areas",
        secondaryList: [
          "Family doctors often provide pregnancy care",
          "Some people give birth at a different hospital than where they get prenatal care",
          "Higher-risk pregnancies may involve specialists",
        ],
        closing:
          "Knowing the plan ahead of time helps reduce stress and surprises.\nIt is okay to ask these questions more than once.",
      },
      {
        title: "Prenatal Care Locations",
        subtitle: "Where to get care nearby",
        intro: "See the resource guide for more information.",
        introLink: "/resources",
        listTitle: "Kearney",
        listItems: [
          "CHI Health Good Samaritan (Maternity Center) — Labor & delivery + postpartum care (regional maternity center).",
          "Kearney Regional Medical Center (Pregnancy & Baby / Maternity + NICU) — Labor & delivery with Level II NICU and lactation support.",
          "Family Practice Associates (Kearney) — Prenatal care with doctors who deliver at Kearney hospitals.",
          "Contemporary Obstetrics & Gynecology (Kearney) — OB/GYN clinic providing pregnancy care and women’s health services.",
        ],
        secondaryTitle: "Lexington",
        secondaryList: [
          "Lexington Regional Health Center (Obstetrics / Labor & Delivery) — Prenatal care through delivery and postpartum support.",
          "Lexington Women’s Care — OB/GYN clinic with routine and high-risk obstetrics care.",
          "Plum Creek Medical Group (Lexington) — Family medicine clinic; can provide prenatal referrals/coordination.",
        ],
        tertiaryTitle: "Cozad, Gothenburg & North Platte",
        tertiaryList: [
          "Cozad Community Medical Clinic — Family medicine clinic that includes prenatal care.",
          "Gothenburg Health (Women’s Health / OB Services) — Prenatal care, labor & delivery, postpartum + lactation support.",
          "Great Plains Health (Pregnancy & Childbirth / Maternity Services) — Prenatal care and hospital-based birth services in North Platte.",
          "North Platte OBGYN, PC — OB/GYN clinic providing pregnancy care and deliveries (women’s health).",
        ],
        quaternaryTitle: "High-Risk Pregnancy Care",
        quaternaryList: [
          "CHI Health Maternal–Fetal Medicine (TeleMFM) — Specialist support for high-risk pregnancies (often via telehealth).",
        ],
      },
      {
        title: "What Prenatal Care Usually Looks Like",
        subtitle: "What happens during pregnancy care",
        intro: [
          "Prenatal care means regular check-ups during pregnancy to help keep you and your baby healthy.",
          "You do not need to understand medical words to get good care.",
          "This section explains what usually happens, so you know what to expect — and what to ask for.",
        ],
        subAccordion: [
          {
            title: "Starting Care & Early Visits",
            intro:
              "Most people have their first prenatal visit in the first few months of pregnancy.",
            listTitle: "At the first visit, the clinic may:",
            listItems: [
              "Confirm the pregnancy",
              "Estimate your due date",
              "Ask about your health, past pregnancies, and medicines",
              "Check your blood pressure and weight",
              "Order blood and urine tests",
              "Sometimes do an early ultrasound",
            ],
            secondaryTitle: "You can ask:",
            secondaryList: [
              "“What is my due date?”",
              "“When should I come back?”",
              "“Who do I call if I have questions?”",
            ],
          },
          {
            title: "Regular Visits & Common Checks",
            intro:
              "After the first visit, most people have regular check-ups during pregnancy.",
            listTitle: "At many visits, they usually:",
            listItems: [
              "Check your blood pressure",
              "Check your weight",
              "Check your urine",
              "Listen to the baby’s heartbeat",
              "Ask how you are feeling",
              "Give time for questions",
            ],
            closing:
              "If something is never being checked, it is okay to ask why.",
          },
          {
            title: "Tests, Ultrasounds & Vaccines",
            intro:
              "During pregnancy, some tests are usually offered to check your health and the baby’s growth.",
            listTitle: "Many people have:",
            listItems: [
              "Blood tests early in pregnancy",
              "An ultrasound around the middle of pregnancy (around 20 weeks)",
              "A diabetes test later in pregnancy",
              "Vaccines that protect you and your baby",
            ],
            secondaryTitle: "Some tests are optional. You can ask:",
            secondaryList: [
              "“What is this test for?”",
              "“Do I have to decide today?”",
            ],
          },
          {
            title: "Changes in Your Body Are Normal",
            intro: "Pregnancy causes many body changes.",
            listTitle: "Common changes include:",
            listItems: [
              "Tiredness",
              "Nausea or vomiting",
              "Body aches",
              "Heartburn",
              "Swelling",
              "Mood changes",
            ],
            closing:
              "You are not weak or doing something wrong.\nAlways tell your provider how you are feeling.",
          },
          {
            title: "When to Get Help Right Away",
            listTitle: "Do not wait for your next appointment if you have:",
            listItems: [
              "Vaginal bleeding",
              "Strong belly pain",
              "Fever",
              "Severe headache or vision problems",
              "Sudden swelling of the face or hands",
              "Trouble breathing",
              "Fluid leaking from the vagina",
              "The baby moving much less than before (later in pregnancy)",
            ],
            secondaryTitle: "You can go to:",
            secondaryList: ["Your clinic", "Urgent care", "Any emergency room"],
            closing: "You cannot be turned away for lack of insurance.",
          },
          {
            title: "How You Help Your Care",
            listTitle: "You help your care by:",
            listItems: [
              "Going to visits when you can",
              "Asking questions",
              "Speaking up if something feels wrong",
              "Asking for help when things are confusing",
            ],
            closing: "Good prenatal care is something you deserve.",
          },
        ],
      },
    ],
  },
  section3: {
    title: "Everyday Pregnancy Care",
    description: [
      "This section focuses on what many people do day to day to support their health during pregnancy.",
      "It covers food, rest, movement, medicines, stress, and common changes in your body. These everyday choices support your health and your baby’s development. You do not need to do everything perfectly. Small, steady steps still help you and your baby.",
      "The Resource Guide includes programs and community supports related to pregnancy wellness that you may find helpful. You can return to this section anytime you have everyday questions or need reassurance.",
    ],
    accordion: [
      {
        title: "Pregnancy Wellness & Everyday Care",
        subtitle: "Simple daily care and when to ask for help",
        intro: [
          "Pregnancy puts extra work on your body.",
          "Small daily choices can help support your health and your baby's health.",
          "You do not need to do everything perfectly.",
          "Doing some of these things is already helping.",
        ],
        subAccordion: [
          {
            title: "Food & Nutrition",
            intro: "Eating regularly helps your body and your baby grow.",
            listTitle: "Try to:",
            listItems: [
              "Eat meals or snacks throughout the day",
              "Include fruits, vegetables, grains, and protein when possible",
              "Drink water often",
            ],
            secondaryTitle: "If food is limited:",
            secondaryList: [
              "Eat what you can",
              "Ask about WIC or food support",
              "Tell your clinic if you are skipping meals",
            ],
            tertiaryTitle: "You can ask:",
            tertiaryList: [
              "“Is there food support I can get during pregnancy?”",
              "“What foods help with nausea or heartburn?”",
            ],
          },
          {
            title: "Prenatal Vitamins",
            intro: "Prenatal vitamins help fill nutrition gaps.",
            listTitle: "Try to:",
            listItems: [
              "Take one prenatal vitamin every day",
              "Take it with food if it upsets your stomach",
              "You can ask for gelatin-free or halal-certified prenatal vitamins",
            ],
            secondaryTitle: "You can say:",
            secondaryList: ["“This vitamin makes me feel sick.”"],
            closing: [
              "If you forget sometimes, that is okay.",
              "If vitamins make you sick, tell your provider.",
              "Other options are available.",
            ],
          },
          {
            title: "Rest, Work, and Daily Life",
            intro: "Your body needs more rest during pregnancy.",
            listTitle: "If possible:",
            listItems: [
              "Rest when you are tired",
              "Sit or lie down during breaks",
              "Avoid lifting very heavy items",
            ],
            secondaryIntro:
              "If you are pregnant during Ramadan, you may have questions about fasting.  Islam allows flexibility during pregnancy if fasting could affect your health or your baby's health.",
            secondaryTitle: "If work is hard on your body:",
            secondaryList: [
              "Tell your provider what your job requires",
              "Ask if a work note or job adjustment is needed",
            ],
            tertiaryTitle: "You can say:",
            tertiaryList: [
              "“My job is very physical.”",
              "“I’m exhausted all the time.”",
            ],
          },
          {
            title: "Movement & Exercise",
            intro: "Gentle movement is usually healthy during pregnancy.",
            listTitle: "Many people benefit from:",
            listItems: ["Walking", "Stretching", "Light daily activity"],
            secondaryTitle: "Stop and get help if you feel:",
            secondaryList: [
              "Dizziness",
              "Chest pain",
              "Strong pain",
              "Bleeding",
            ],
            tertiaryTitle: "You can ask:",
            tertiaryList: ["“What kind of movement is safe for me?”"],
          },
          {
            title: "Stress, Mood & Emotional Health",
            intro: "Pregnancy can bring:",
            listItems: [
              "Stress",
              "Worry",
              "Sadness",
              "Anger",
              "Trouble sleeping",
            ],
            secondaryTitle:
              "These feelings are common — and help is available.",
            tertiaryTitle: "Tell your provider if you feel:",
            tertiaryList: ["Overwhelmed", "Very sad", "Very anxious", "Unsafe"],
            quaternaryTitle: "You can say:",
            quaternaryList: [
              "“I don’t feel like myself.”",
              "“I'm having a hard time coping.”",
            ],
            closing: "You deserve support during pregnancy.",
          },
          {
            title: "Infections & Staying Healthy",
            listTitle: "To help protect yourself:",
            listItems: [
              "Wash your hands often",
              "Avoid people who are very sick when possible",
              "Go to appointments and get recommended vaccines",
            ],
            secondaryTitle: "Call or seek care if you have:",
            secondaryList: [
              "Fever",
              "Burning with urination",
              "Strong belly pain",
              "Vaginal bleeding",
            ],
          },
        ],
      },
    ],
  },
  section4: {
    title: "Paying for Care, Paperwork & Support",
    description: [
      "This section helps explain how to pay for care and get support.",
      "Many people need help with insurance, paperwork, or finding food, housing, transportation, or safety support. That is common — and help is available.",
      "This section explains how systems work and what kinds of help may be available.",
      "The Resource Guide lists organizations that can help with coverage, benefits, paperwork, and other support.",
      "You can look at one part at a time. You do not need to understand everything to start.",
    ],
    accordion: [
      {
        title: "Getting Health Coverage During Pregnancy",
        subtitle: "How to get coverage and what it helps pay for",
        intro: [
          "There are programs that can help pay for medical care and food during pregnancy.",
          "You do not need to understand all programs to get started. Help may be available even if your immigration situation is complicated.",
          "Different programs help in different ways:",
        ],
        listItems: [
          "Some provide health insurance",
          "Some provide pregnancy-only care",
          "Some help with food and nutrition",
        ],
        closing:
          "You may hear these programs called Medicaid, CHIP, or WIC. More details are available below.",
      },
      {
        title: "Support for Special Situations",
        subtitle: "Extra help is common and available.",
        intro:
          "Pregnancy can bring new worries about money, housing, food, safety, or paperwork.",
        listTitle: "Help is available for:",
        listItems: [
          "Immigration concerns",
          "Food support",
          "Housing help",
          "Transportation",
          "Safety concerns",
        ],
        secondaryTitle: "Important to know",
        secondaryList: [
          "Clinics and hospitals do not report immigration status",
          "Asking for help will not affect your medical care",
          "You can ask for help even if you are not sure what you need",
        ],
        closing: "You do not have to handle this alone.",
      },
      {
        title: "More Help With Coverage & Paperwork",
        subtitle: "Programs, applications, documents, and next steps",
        secondaryTitle: "Understanding Medicaid, CHIP & WIC",
        secondaryIntro: "Different programs help in different ways:",
        secondaryList: [
          "Medicaid – Health insurance that helps pay for medical care",
          "CHIP (Children’s Health Insurance Program) – Health coverage for children and some pregnant people",
          "WIC (Women, Infants, and Children) – Food, nutrition, and breastfeeding support",
        ],
        closing:
          "You do not need to know which program fits you before you apply.",
      },
      {
        title: "How to Apply for Health Coverage",
        subtitle: "Ways to apply and get help",
        intro: [
          "Ways to apply and get help",
          "There is one main application for Medicaid and CHIP in Nebraska.",
          "You do not need to know which program you qualify for before you apply.",
          "The state will review your information and decide what fits your situation.",
          "Many people need help with the application — that is okay.",
          "You can apply in different ways:",
        ],
        listItems: ["Online", "By phone", "In person"],
        secondaryIntro: [
          "You can choose the option that works best for you.",
          "Apply even if documents are missing.",
          "Staff can tell you what to bring later.",
        ],
        tertiaryTitle: "You can say:",
        tertiaryIntro:
          "“I want to apply for pregnancy health coverage. Can you help me?”",
        subAccordion: [
          {
            title: "Health Insurance Marketplace",
            intro:
              "Some pregnant people use insurance through the Health Insurance Marketplace (Healthcare.gov).",
            listItems: [
              "Are self-employed",
              "Do not qualify for Medicaid or CHIP",
              "Want coverage that continues after your baby is born",
            ],
            closing:
              "Marketplace plans may have monthly costs, but many people qualify for financial help.\nHelp is available to compare plans and apply.",
          },
          {
            title: "Documents You May Be Asked For",
            intro: [
              "You are not alone. Take small, clear steps.",
              "Documents are often requested after you apply.",
              "It is okay if you do not have everything.",
            ],
            listTitle: "Pregnancy Verification Letter",
            listItems: [
              "Confirms that you are pregnant.",
              "You can get this from: Two Rivers Public Health Department or a clinic appointment",
              "Cost-saving tip: Public Health may provide this letter without a clinic visit.",
              "You can say: “I need a pregnancy verification letter to apply for health coverage.”",
            ],
            secondaryTitle: "A Form of ID",
            secondaryIntro: "Any document with your name, such as:",
            secondaryList: [
              "Driver’s license or state ID",
              "Passport (from any country)",
              "School, work, or consular ID",
            ],
            tertiaryTitle: "Proof of Address",
            tertiaryIntro: "Shows where you live now, such as:",
            tertiaryList: [
              "Mail with your name",
              "Lease or utility bill",
              "Note from the person you live with or a shelter",
            ],
            quaternaryTitle: "Proof of Income (if available)",
            quaternaryList: [
              "Examples: Pay stubs or employer letter",
              "Bank records",
              "Statement of no income",
            ],
            closing: "Apply even if documents are missing.",
          },
          {
            title: "While You Are Waiting for Coverage",
            intro: "You may not get an insurance card right away.",
            listTitle: "Important to know:",
            listItems: [
              "Approval may happen after care has started",
              "Medicaid may pay for care up to 3 months before approval (retroactive Medicaid)",
            ],
            secondaryTitle: "Because of this:",
            secondaryList: [
              "You may be asked to pay cash while waiting",
              "Tell clinics that you have applied for Medicaid",
              "Keep receipts in case coverage is approved later",
            ],
            tertiaryTitle: "If you need emergency care:",
            tertiaryList: [
              "Go to any emergency room",
              "You cannot be turned away for lack of insurance",
            ],
            closing: "Your health and safety come first.",
          },
          {
            title: "Important Information to Keep",
            intro:
              "If you can, keep this information with you — especially if you move or change clinics.",
            listTitle: "Important papers",
            listItems: [
              "Pregnancy verification letter",
              "Insurance or Medicaid letters",
              "ID, address, and income papers (if available)",
              "Prenatal visit notes",
              "Ultrasound reports",
              "Lab results",
            ],
            secondaryTitle: "Important pregnancy information",
            secondaryList: [
              "Due date (or best estimate)",
              "Pregnancy problems",
              "Past pregnancy or delivery problems",
            ],
            tertiaryTitle: "Medications and allergies",
            tertiaryList: [
              "Medicines you take",
              "Vitamins or supplements",
              "Drug or food allergies",
            ],
            quaternaryTitle: "Helpful tips",
            quaternaryList: [
              "Take photos of papers on your phone",
              "Keep papers in one place",
            ],
            closing: "Ask for copies before leaving appointments",
          },
        ],
      },
    ],
  },
};

const sectionDataEs = {
  section1: {
    title: "Inicio, Seguridad y Decisiones sobre el Embarazo",
    description: [
      "Si está embarazada, cree que podría estarlo o acaba de enterarse, no está sola.",
      "Las personas tienen muchos sentimientos diferentes al comienzo del embarazo.",
      "Esta sección le ayuda a comenzar. Aquí encontrará qué hacer primero, cuándo buscar ayuda de inmediato e información sobre las opciones de embarazo.",
      "Esta sección también incluye información sobre señales de advertencia, preocupaciones urgentes y situaciones especiales.",
      "La Guía de Recursos puede conectarla con organizaciones si necesita pruebas, ayuda urgente o apoyo adicional.",
      "No tiene que decidir todo hoy.",
      "Lea lo que le resulte útil en este momento.",
    ],
    accordion: [
      {
        title: "Primeros Pasos y Sus Opciones",
        subtitle: "No está sola. Dé pasos pequeños y claros.",
        intro: [
          "Cuando comienza el embarazo, muchas personas están aprendiendo cómo funciona el sistema de salud por primera vez.",
          "Esta guía está aquí para ayudarle a dar pasos pequeños y claros, uno a la vez.",
        ],
        listTitle: "Hay ayuda disponible especialmente si:",
        listItems: [
          "Aún no tiene seguro médico",
          "Su estado migratorio es complicado",
          "No habla bien inglés",
          "Se ha mudado recientemente o podría necesitar mudarse de nuevo",
        ],
        closing: "Puede comenzar donde está. Hay apoyo disponible.",
      },
      {
        title: "Si Cree que Podría Estar Embarazada",
        subtitle: "Cómo hacerse una prueba y qué hacer después",
        intro: [
          "Puede comprar una prueba de embarazo en la mayoría de las tiendas.",
          "También puede hacerse una prueba de embarazo en:",
        ],
        listItems: [
          "El Departamento de Salud Pública de Two Rivers (gratis o a bajo costo)",
          "Clínicas locales (ver el Directorio de Recursos)",
        ],
        secondaryTitle: "Si la prueba es positiva:",
        secondaryList: [
          "Intente concertar una cita prenatal dentro de 1 o 2 semanas",
          "Comience a tomar una vitamina prenatal (no necesita receta)",
          "Solicite cobertura de salud si la necesita",
        ],
        tertiaryTitle: "Solicitud de Cobertura de Salud Durante el Embarazo",
        tertiaryLink: "#section-4-3",
        closing: [
          "Para solicitar cobertura de salud, es posible que necesite una carta de verificación de embarazo de un proveedor de atención médica.",
          "Consejo para ahorrar costos: El Departamento de Salud Pública de Two Rivers puede proporcionar la prueba de embarazo y la carta de verificación sin una visita a la clínica.",
        ],
      },
      {
        title: "Si Necesita Atención de Inmediato",
        subtitle: "La atención de emergencia es lo primero",
        intro: [
          "Si tiene dolor intenso, sangrado abundante, fiebre o se siente insegura, busque atención de inmediato.",
          "Vaya a cualquier sala de emergencias.",
        ],
        listTitle: "No pueden rechazarla debido a:",
        listItems: [
          "Estado del seguro",
          "Estado migratorio",
          "Capacidad de pago",
        ],
      },
      {
        title: "¿Necesita Ayuda Extra Ahora Mismo?",
        subtitle: "Apoyo adicional durante el embarazo",
        listTitle: "Hay ayuda disponible si le preocupa:",
        listItems: [
          "Comida",
          "Vivienda",
          "Transporte",
          "Seguridad",
          "Preocupaciones de inmigración",
        ],
        secondaryTitle: "No tiene que manejar esto sola.",
        tertiaryTitle:
          "Hay apoyo disponible. Consulte la guía de recursos para ver los recursos locales.",
        tertiaryLink: "/resources",
      },
      {
        title: "Sus opciones de embarazo",
        subtitle: "Aprenda sobre crianza, adopción y opciones de embarazo",
        intro: [
          "Las personas eligen diferentes caminos durante el embarazo.",
          "Algunas personas:",
        ],
        listItems: [
          "Continúan el embarazo y crían al bebé",
          "Hacen un plan de adopción",
          "Deciden interrumpir el embarazo",
        ],
        secondaryTitle: "Adopción",
        secondaryIntro: "La adopción es una opción legal. Usted puede:",
        secondaryList: [
          "Elegir una adopción abierta, semiabierta o cerrada (donde los padres biológicos están totalmente involucrados, parcialmente involucrados o no involucrados)",
          "Elegir la familia para su bebé",
          "Obtener ayuda con algunos gastos relacionados con el embarazo",
          "Cambiar de opinión hasta que la adopción finalice legalmente",
        ],
        tertiaryTitle: "Aborto",
        tertiaryIntro:
          "La ley de Nebraska limita el aborto después de las 12 semanas de embarazo, con pocas excepciones.",
        tertiaryList: [
          "Comuníquese con Planned Parenthood para recibir asesoramiento sobre el aborto",
        ],
      },
      {
        title: "Por Qué Importa la Atención Prenatal Temprana",
        subtitle: "Cómo la atención temprana le ayuda a usted y a su bebé",
        intro: "Ver a un proveedor prenatal temprano ayuda a:",
        listItems: [
          "Proteger su salud",
          "Proteger la salud de su bebé",
          "Encontrar problemas temprano",
          "Conectarla con seguro, ayuda alimentaria y otro tipo de apoyo",
        ],
        closing:
          "Intente comenzar la atención prenatal tan pronto como pueda.\nTome una vitamina prenatal todos los días si es posible.",
      },
    ],
  },
  section2: {
    title: "Atención Prenatal: Acceso y Cómo Funciona",
    description: [
      "Esta sección explica cómo funciona la atención prenatal.",
      "Aprenderá:",
    ],
    descriptionListItems: [
      "Dónde obtener atención prenatal",
      "Qué sucede generalmente en las visitas",
      "Cómo es la atención en diferentes partes del embarazo",
    ],
    descriptionClosing: [
      "Esta sección también incluye información sobre la atención después del parto.",
      "También encontrará orientación sobre cómo hacer preguntas, obtener ayuda para entender la información y hablar durante la atención. Usted es parte del equipo de atención.",
      "La Guía de Recursos puede ayudarla a encontrar clínicas y servicios para la atención prenatal y posparto.",
      "Esta sección se puede utilizar como referencia durante todo el embarazo y después del nacimiento.",
    ],
    accordion: [
      {
        title: "Abogar por Usted Misma en la Atención Médica",
        subtitle: "Tiene derechos durante la atención médica.",
        listTitle: "Tiene derecho a:",
        listItems: [
          "Hacer preguntas",
          "Pedir un intérprete",
          "Llevar a alguien con usted",
          "Pedir copias de sus registros",
          "Cambiar de proveedor",
        ],
        secondaryTitle: "Puede decir:",
        secondaryList: [
          "“¿Puede explicar eso de nuevo?”",
          "“Necesito un intérprete.”",
          "“Algo no se siente bien.”",
        ],
      },
      {
        title: "Elegir un Proveedor de Atención Prenatal",
        subtitle: "Dónde recibirá atención y dará a luz",
        intro: [
          "No todas las clínicas brindan atención prenatal.",
          "No todos los hospitales atienden partos.",
          "En una emergencia, vaya a la sala de emergencias.",
          "Ayuda preguntar con anticipación dónde dará a luz.",
        ],
        listTitle: "Al llamar a una clínica, puede preguntar:",
        listItems: [
          "¿Brindan atención prenatal?",
          "¿Atienden partos?",
          "¿Dónde daría a luz?",
          "¿A qué hospital debo ir?",
          "¿Qué pasa si el parto comienza por la noche?",
          "¿A quién llamo?",
          "¿Tienen un intérprete?",
        ],
        secondaryTitle: "En Áreas Rurales",
        secondaryList: [
          "Los médicos de familia a menudo brindan atención durante el embarazo",
          "Algunas personas dan a luz en un hospital diferente al que reciben atención prenatal",
          "Los embarazos de mayor riesgo pueden involucrar especialistas",
        ],
        closing:
          "Conocer el plan con anticipación ayuda a reducir el estrés y las sorpresas.\nEstá bien hacer estas preguntas más de una vez.",
      },
      {
        title: "Ubicaciones de Atención Prenatal",
        subtitle: "Dónde obtener atención cercana",
        intro: "Consulte la guía de recursos para obtener más información.",
        introLink: "/resources",
        listTitle: "Kearney",
        listItems: [
          "CHI Health Good Samaritan (Centro de Maternidad) — Trabajo de parto y parto + atención posparto (centro regional de maternidad).",
          "Kearney Regional Medical Center (Embarazo y Bebé / Maternidad + NICU) — Trabajo de parto y parto con NICU Nivel II y apoyo de lactancia.",
          "Family Practice Associates (Kearney) — Atención prenatal con médicos que atienden partos en hospitales de Kearney.",
          "Contemporary Obstetrics & Gynecology (Kearney) — Clínica de OB/GYN que brinda atención durante el embarazo y servicios de salud para la mujer.",
        ],
        secondaryTitle: "Lexington",
        secondaryList: [
          "Lexington Regional Health Center (Obstetricia / Trabajo de Parto y Parto) — Atención prenatal hasta el parto y apoyo posparto.",
          "Lexington Women’s Care — Clínica de OB/GYN con atención obstétrica de rutina y de alto riesgo.",
          "Plum Creek Medical Group (Lexington) — Clínica de medicina familiar; puede proporcionar referencias/coordinación prenatal.",
        ],
        tertiaryTitle: "Cozad, Gothenburg y North Platte",
        tertiaryList: [
          "Cozad Community Medical Clinic — Clínica de medicina familiar que incluye atención prenatal.",
          "Gothenburg Health (Salud de la Mujer / Servicios de Obstetricia) — Atención prenatal, trabajo de parto y parto, posparto + apoyo de lactancia.",
          "Great Plains Health (Embarazo y Parto / Servicios de Maternidad) — Atención prenatal y servicios de parto en el hospital en North Platte.",
          "North Platte OBGYN, PC — Clínica de OB/GYN que brinda atención durante el embarazo y partos (salud de la mujer).",
        ],
        quaternaryTitle: "Atención de Embarazo de Alto Riesgo",
        quaternaryList: [
          "CHI Health Medicina Materno-Fetal (TeleMFM) — Apoyo especializado para embarazos de alto riesgo (a menudo vía telesalud).",
        ],
      },
      {
        title: "Cómo Es Generalmente la Atención Prenatal",
        subtitle: "Qué sucede durante la atención del embarazo",
        intro: [
          "La atención prenatal significa chequeos regulares durante el embarazo para ayudar a mantenerla a usted y a su bebé saludables.",
          "No necesita entender palabras médicas para recibir una buena atención.",
          "Esta sección explica lo que sucede generalmente, para que sepa qué esperar y qué pedir.",
        ],
        subAccordion: [
          {
            title: "Comenzando la Atención y Visitas Tempranas",
            intro:
              "La mayoría de las personas tienen su primera visita prenatal en los primeros meses de embarazo.",
            listTitle: "En la primera visita, la clínica puede:",
            listItems: [
              "Confirmar el embarazo",
              "Estimar su fecha de parto",
              "Preguntar sobre su salud, embarazos pasados y medicamentos",
              "Verificar su presión arterial y peso",
              "Ordenar análisis de sangre y orina",
              "A veces hacer una ecografía temprana",
            ],
            secondaryTitle: "Puede preguntar:",
            secondaryList: [
              "“¿Cuál es mi fecha de parto?”",
              "“¿Cuándo debo volver?”",
              "“¿A quién llamo si tengo preguntas?”",
            ],
          },
          {
            title: "Visitas Regulares y Chequeos Comunes",
            intro:
              "Después de la primera visita, la mayoría de las personas tienen chequeos regulares durante el embarazo.",
            listTitle: "En muchas visitas, generalmente:",
            listItems: [
              "Verifican su presión arterial",
              "Verifican su peso",
              "Verifican su orina",
              "Escuchan los latidos del corazón del bebé",
              "Preguntan cómo se siente",
              "Dan tiempo para preguntas",
            ],
            closing: "Si algo nunca se revisa, está bien preguntar por qué.",
          },
          {
            title: "Pruebas, Ecografías y Vacunas",
            intro:
              "Durante el embarazo, generalmente se ofrecen algunas pruebas para verificar su salud y el crecimiento del bebé.",
            listTitle: "Muchas personas tienen:",
            listItems: [
              "Análisis de sangre al principio del embarazo",
              "Una ecografía a mediados del embarazo (alrededor de las 20 semanas)",
              "Una prueba de diabetes más adelante en el embarazo",
              "Vacunas que la protegen a usted y a su bebé",
            ],
            secondaryTitle: "Algunas pruebas son opcionales. Puede preguntar:",
            secondaryList: [
              "“¿Para qué es esta prueba?”",
              "“¿Tengo que decidir hoy?”",
            ],
          },
          {
            title: "Los Cambios en Su Cuerpo Son Normales",
            intro: "El embarazo causa muchos cambios en el cuerpo.",
            listTitle: "Los cambios comunes incluyen:",
            listItems: [
              "Cansancio",
              "Náuseas o vómitos",
              "Dolores corporales",
              "Acidez estomacal",
              "Hinchazón",
              "Cambios de humor",
            ],
            closing:
              "No es débil ni está haciendo algo mal.\nSiempre dígale a su proveedor cómo se siente.",
          },
          {
            title: "Cuándo Obtener Ayuda de Inmediato",
            listTitle: "No espere a su próxima cita si tiene:",
            listItems: [
              "Sangrado abundante",
              "Dolor abdominal fuerte",
              "Fiebre",
              "Dolor de cabeza intenso o problemas de visión",
              "Hinchazón repentina de la cara o las manos",
              "Dificultad para respirar",
              "Líquido saliendo de la vagina",
              "El bebé moviéndose mucho menos que antes (más adelante en el embarazo)",
            ],
            secondaryTitle: "Puede ir a:",
            secondaryList: [
              "Su clínica",
              "Atención de urgencia",
              "Cualquier sala de emergencias",
            ],
            closing: "No pueden rechazarla por falta de seguro.",
          },
          {
            title: "Cómo Ayuda a Su Atención",
            listTitle: "Usted ayuda a su atención al:",
            listItems: [
              "Ir a las visitas cuando pueda",
              "Hacer preguntas",
              "Hablar si algo se siente mal",
              "Pedir ayuda cuando las cosas son confusas",
            ],
            closing: "Una buena atención prenatal es algo que usted merece.",
          },
        ],
      },
    ],
  },
  section3: {
    title: "Cuidados Diarios del Embarazo",
    description: [
      "Esta sección se centra en lo que muchas personas hacen día a día para apoyar su salud durante el embarazo.",
      "Cubre alimentación, descanso, movimiento, medicamentos, estrés y cambios comunes en su cuerpo. Estas elecciones diarias apoyan su salud y el desarrollo de su bebé. No necesita hacer todo perfectamente. Los pasos pequeños y constantes ayudan a usted y a su bebé.",
      "La Guía de Recursos incluye programas y apoyos comunitarios relacionados con el bienestar en el embarazo que pueden resultarle útiles. Puede volver a esta sección en cualquier momento que tenga preguntas diarias o necesite tranquilidad.",
    ],
    accordion: [
      {
        title: "Bienestar en el Embarazo y Cuidados Diarios",
        subtitle: "Cuidado diario simple y cuándo pedir ayuda",
        intro: [
          "El embarazo pone trabajo extra en su cuerpo.",
          "Pequeñas elecciones diarias pueden ayudar a apoyar su salud y la salud de su bebé.",
          "No necesita hacer todo perfectamente.",
          "Hacer algunas de estas cosas ya está ayudando.",
        ],
        subAccordion: [
          {
            title: "Alimentos y Nutrición",
            intro:
              "Comer regularmente ayuda a que su cuerpo y su bebé crezcan.",
            listTitle: "Trate de:",
            listItems: [
              "Comer comidas o refrigerios durante todo el día",
              "Incluir frutas, verduras, granos y proteínas cuando sea posible",
              "Beber agua con frecuencia",
            ],
            secondaryTitle: "Si la comida es limitada:",
            secondaryList: [
              "Coma lo que pueda",
              "Pregunte sobre WIC o apoyo alimentario",
              "Dígale a su clínica si se está saltando comidas",
            ],
            tertiaryTitle: "Puede preguntar:",
            tertiaryList: [
              "“¿Hay apoyo alimentario que pueda obtener durante el embarazo?”",
              "“¿Qué alimentos ayudan con las náuseas o la acidez estomacal?”",
            ],
          },
          {
            title: "Vitaminas Prenatales",
            intro:
              "Las vitaminas prenatales ayudan a llenar los vacíos nutricionales.",
            listTitle: "Trate de:",
            listItems: [
              "Tomar una vitamina prenatal todos los días",
              "Tomarla con comida si le molesta el estómago",
              "Puede pedir vitaminas prenatales sin gelatina o con certificación halal",
            ],
            secondaryTitle: "Puede decir:",
            secondaryList: ["“Esta vitamina me hace sentir enferma.”"],
            closing: [
              "Si se olvida a veces, está bien.",
              "Si las vitaminas la enferman, dígale a su proveedor.",
              "Hay otras opciones disponibles.",
            ],
          },
          {
            title: "Descanso, Trabajo y Vida Diaria",
            intro: "Su cuerpo necesita más descanso durante el embarazo.",
            listTitle: "Si es posible:",
            listItems: [
              "Descanse cuando esté cansada",
              "Siéntese o acuéstese durante los descansos",
              "Evite levantar objetos muy pesados",
            ],
            secondaryIntro:
              "Si está embarazada durante el Ramadán, es posible que tenga preguntas sobre el ayuno. El Islam permite flexibilidad durante el embarazo si el ayuno pudiera afectar su salud o la de su bebé.",
            secondaryTitle: "Si el trabajo es duro para su cuerpo:",
            secondaryList: [
              "Dígale a su proveedor lo que requiere su trabajo",
              "Pregunte si se necesita una nota de trabajo o un ajuste laboral",
            ],
            tertiaryTitle: "Puede decir:",
            tertiaryList: [
              "“Mi trabajo es muy físico.”",
              "“Estoy agotada todo el tiempo.”",
            ],
          },
          {
            title: "Movimiento y Ejercicio",
            intro:
              "El movimiento suave suele ser saludable durante el embarazo.",
            listTitle: "Muchas personas se benefician de:",
            listItems: ["Caminar", "Estirarse", "Actividad diaria ligera"],
            secondaryTitle: "Deténgase y busque ayuda si siente:",
            secondaryList: [
              "Mareos",
              "Dolor en el pecho",
              "Dolor fuerte",
              "Sangrado",
            ],
            tertiaryTitle: "Puede preguntar:",
            tertiaryList: ["“¿Qué tipo de movimiento es seguro para mí?”"],
          },
          {
            title: "Estrés, Estado de Ánimo y Salud Emocional",
            intro: "El embarazo puede traer:",
            listItems: [
              "Estrés",
              "Preocupación",
              "Tristeza",
              "Ira",
              "Problemas para dormir",
            ],
            secondaryTitle:
              "Estos sentimientos son comunes — y hay ayuda disponible.",
            tertiaryTitle: "Dígale a su proveedor si se siente:",
            tertiaryList: ["Abrumada", "Muy triste", "Muy ansiosa", "Insegura"],
            quaternaryTitle: "Puede decir:",
            quaternaryList: [
              "“No me siento como yo misma.”",
              "“Me está costando sobrellevarlo.”",
            ],
            closing: "Usted merece apoyo durante el embarazo.",
          },
          {
            title: "Infecciones y Mantenerse Saludable",
            listTitle: "Para ayudar a protegerse:",
            listItems: [
              "Lávese las manos con frecuencia",
              "Evite a las personas que están muy enfermas cuando sea posible",
              "Vaya a las citas y obtenga las vacunas recomendadas",
            ],
            secondaryTitle: "Llame o busque atención si tiene:",
            secondaryList: [
              "Fiebre",
              "Ardor al orinar",
              "Dolor abdominal fuerte",
              "Sangrado vaginal",
            ],
          },
        ],
      },
    ],
  },
  section4: {
    title: "Pago de la Atención, Trámites y Apoyo",
    description: [
      "Esta sección ayuda con el seguro, el papeleo y el apoyo adicional.",
      "Muchas personas necesitan ayuda para pagar la atención, entender los formularios o encontrar apoyo para alimentos, vivienda, transporte o seguridad.",
      "Eso es común — y hay ayuda disponible.",
      "Puede mirar una parte a la vez.",
      "No necesita entender todo para comenzar.",
    ],
    accordion: [
      {
        title: "Obtención de Cobertura de Salud Durante el Embarazo",
        subtitle: "Cómo obtener cobertura y qué ayuda a pagar",
        intro: [
          "Hay programas que pueden ayudar a pagar la atención médica y los alimentos durante el embarazo.",
          "No necesita entender todos los programas para comenzar. Puede haber ayuda disponible incluso si su situación migratoria es complicada.",
          "Diferentes programas ayudan de diferentes maneras:",
        ],
        listItems: [
          "Algunos proporcionan seguro médico",
          "Algunos proporcionan atención solo para el embarazo",
          "Algunos ayudan con alimentos y nutrición",
        ],
        closing:
          "Es posible que escuche que estos programas se llaman Medicaid, CHIP o WIC. Más detalles están disponibles a continuación.",
      },
      {
        title: "Apoyo para Situaciones Especiales",
        subtitle: "La ayuda adicional es común y está disponible.",
        intro:
          "El embarazo puede traer nuevas preocupaciones sobre dinero, vivienda, comida, seguridad o papeleo.",
        listTitle: "Hay ayuda disponible para:",
        listItems: [
          "Preocupaciones de inmigración",
          "Apoyo alimentario",
          "Ayuda de vivienda",
          "Transporte",
          "Preocupaciones de seguridad",
        ],
        secondaryTitle: "Importante saber",
        secondaryList: [
          "Las clínicas y los hospitales no informan el estado migratorio",
          "Pedir ayuda no afectará su atención médica",
          "Puede pedir ayuda incluso si no está segura de lo que necesita",
        ],
        closing: "No tiene que manejar esto sola.",
      },
      {
        title: "Más Ayuda con Cobertura y Trámites",
        subtitle: "Programas, solicitudes, documentos y próximos pasos",
        secondaryTitle: "Entendiendo Medicaid, CHIP y WIC",
        secondaryIntro: "Diferentes programas ayudan de diferentes maneras:",
        secondaryList: [
          "Medicaid – Seguro médico que ayuda a pagar la atención médica",
          "CHIP (Programa de Seguro Médico para Niños) – Cobertura de salud para niños y algunas personas embarazadas",
          "WIC (Mujeres, Bebés y Niños) – Alimentos, nutrición y apoyo a la lactancia",
        ],
        closing:
          "No necesita saber qué programa le corresponde antes de solicitar.",
      },
      {
        title: "Cómo Solicitar Cobertura de Salud",
        subtitle: "Formas de solicitar y obtener ayuda",
        intro: [
          "Formas de solicitar y obtener ayuda",
          "Hay una solicitud principal para Medicaid y CHIP en Nebraska.",
          "No necesita saber para qué programa califica antes de solicitar.",
          "El estado revisará su información y decidirá qué se ajusta a su situación.",
          "Muchas personas necesitan ayuda con la solicitud — eso está bien.",
          "Puede solicitar de diferentes maneras:",
        ],
        listItems: ["En línea", "Por teléfono", "En persona"],
        secondaryIntro: [
          "Puede elegir la opción que mejor funcione para usted.",
          "Solicite incluso si faltan documentos.",
          "El personal puede decirle qué traer más tarde.",
        ],
        tertiaryTitle: "Puede decir:",
        tertiaryIntro:
          "“Quiero solicitar cobertura de salud para el embarazo. ¿Me pueden ayudar?”",
        subAccordion: [
          {
            title: "Mercado de Seguros Médicos",
            intro:
              "Algunas personas embarazadas usan un seguro a través del Mercado de Seguros Médicos (Healthcare.gov).",
            listItems: [
              "Trabajan por cuenta propia",
              "No califican para Medicaid o CHIP",
              "Quieren cobertura que continúe después de que nazca su bebé",
            ],
            closing:
              "Los planes del Mercado pueden tener costos mensuales, pero muchas personas califican para ayuda financiera.\nHay ayuda disponible para comparar planes y solicitar.",
          },
          {
            title: "Documentos Que Se Le Pueden Pedir",
            intro: [
              "No está sola. Dé pasos pequeños y claros.",
              "Los documentos a menudo se solicitan después de presentar la solicitud.",
              "Está bien si no tiene todo.",
            ],
            listTitle: "Carta de Verificación de Embarazo",
            listItems: [
              "Confirma que está embarazada.",
              "Puede obtener esto de: El Departamento de Salud Pública de Two Rivers o una cita en la clínica",
              "Consejo para ahorrar costos: Salud Pública puede proporcionar esta carta sin una visita a la clínica.",
              "Puede decir: “Necesito una carta de verificación de embarazo para solicitar cobertura de salud.”",
            ],
            secondaryTitle: "Una forma de identificación",
            secondaryIntro: "Cualquier documento con su nombre, como:",
            secondaryList: [
              "Licencia de conducir o identificación estatal",
              "Pasaporte (de cualquier país)",
              "Identificación escolar, laboral o consular",
            ],
            tertiaryTitle: "Prueba de Dirección",
            tertiaryIntro: "Muestra dónde vive ahora, como:",
            tertiaryList: [
              "Correo con su nombre",
              "Contrato de arrendamiento o factura de servicios públicos",
              "Nota de la persona con la que vive o un refugio",
            ],
            quaternaryTitle: "Prueba de Ingresos (si está disponible)",
            quaternaryList: [
              "Ejemplos: Talones de pago o carta del empleador",
              "Registros bancarios",
              "Declaración de no ingresos",
            ],
            closing: "Solicite incluso si faltan documentos.",
          },
          {
            title: "Mientras Espera la Cobertura",
            intro:
              "Es posible que no obtenga una tarjeta de seguro de inmediato.",
            listTitle: "Importante saber:",
            listItems: [
              "La aprobación puede ocurrir después de que haya comenzado la atención",
              "Medicaid puede pagar la atención hasta 3 meses antes de la aprobación (Medicaid retroactivo)",
            ],
            secondaryTitle: "Debido a esto:",
            secondaryList: [
              "Se le puede pedir que pague en efectivo mientras espera",
              "Dígale a las clínicas que ha solicitado Medicaid",
              "Guarde los recibos en caso de que la cobertura se apruebe más tarde",
            ],
            tertiaryTitle: "Si necesita atención de emergencia:",
            tertiaryList: [
              "Vaya a cualquier sala de emergencias",
              "No pueden rechazarla por falta de seguro",
            ],
            closing: "Su salud y seguridad son lo primero.",
          },
          {
            title: "Información Importante para Guardar",
            intro:
              "Si puede, guarde esta información con usted — especialmente si se muda o cambia de clínica.",
            listTitle: "Papeles importantes",
            listItems: [
              "Carta de verificación de embarazo",
              "Cartas de seguro o Medicaid",
              "Papeles de identificación, dirección e ingresos (si están disponibles)",
              "Notas de visitas prenatales",
              "Informes de ultrasonido",
              "Resultados de laboratorio",
            ],
            secondaryTitle: "Información importante sobre el embarazo",
            secondaryList: [
              "Fecha de parto (o mejor estimación)",
              "Problemas de embarazo",
              "Problemas pasados de embarazo o parto",
            ],
            tertiaryTitle: "Medicamentos y alergias",
            tertiaryList: [
              "Medicamentos que toma",
              "Vitaminas o suplementos",
              "Alergias a medicamentos o alimentos",
            ],
            quaternaryTitle: "Consejos útiles",
            quaternaryList: [
              "Tome fotos de los papeles en su teléfono",
              "Guarde los papeles en un solo lugar",
            ],
            closing: "Pida copias antes de salir de las citas",
          },
        ],
      },
    ],
  },
};

const sectionDataSo = {
  section1: {
    title: "Bilowga, Badbaadada & Go'aamada Uurka",
    description: [
      "Haddii aad uur leedahay, laga yaabo inaad uur leedahay, ama aad hadda ogaatay inaad uur leedahay, keligaa ma tihid.",
      "Dadku waxay leeyihiin dareemo kala duwan bilowga uurka.",
      "Qaybtani waxay kaa caawinaysaa inaad bilowdo. Halkan waxaad ka heli doontaa waxa marka hore la sameeyo, goorta la helo caawimaad degdeg ah, iyo macluumaad ku saabsan ikhtiyaarka uurka.",
      "Qaybtani waxay sidoo kale ku jirtaa macluumaad ku saabsan calaamadaha digniinta, walaacyada degdegga ah, iyo xaaladaha gaarka ah.",
      "Hagaha Kheyraadka wuxuu kugu xiri karaa ururada haddii aad u baahan tahay baaritaan, caawimaad degdeg ah, ama taageero dheeraad ah.",
      "Ma aha inaad maanta go'aan ka gaarto wax walba.",
      "Akhri waxa waxtar kuu leh hadda.",
    ],
    accordion: [
      {
        title: "Bilaabidda & Ikhtiyaarkaaga",
        subtitle: "Keligaa ma tihid. Qaad tallaabooyin yaryar oo cad.",
        intro: [
          "Marka uurku bilaabmo, dad badan ayaa baranaya sida nidaamka daryeelka caafimaadku u shaqeeyo markii ugu horreysay.",
          "Hagahan ayaa halkan u jooga inuu kaa caawiyo inaad qaaddo tallaabooyin yaryar oo cad, mid mid.",
        ],
        listTitle: "Caawimaad ayaa la heli karaa gaar ahaan haddii:",
        listItems: [
          "Aadan weli lahayn caymis",
          "Xaaladdaada socdaalka ay adag tahay",
          "Aadan si fiican u hadlin Ingiriisiga",
          "Aad dhawaan soo guurtay ama laga yaabo inaad u baahato inaad mar kale guurto",
        ],
        closing:
          "Waxaad ka bilaabi kartaa halka aad joogto. Taageero ayaa la heli karaa.",
      },
      {
        title: "Haddii Aad U Moleyso Inaad Uur Leedahay",
        subtitle: "Sida loo helo baaritaan iyo waxa la sameeyo marka xigta",
        intro: [
          "Waxaad ka iibsan kartaa baaritaanka uurka dukaamada intooda badan.",
          "Waxaad sidoo kale baaritaanka uurka ka heli kartaa:",
        ],
        listItems: [
          "Waaxda Caafimaadka Dadweynaha ee Two Rivers (bilaash ama qiimo jaban)",
          "Rugaha caafimaadka ee maxalliga ah (eeg Hagaha Kheyraadka)",
        ],
        secondaryTitle: "Haddii baaritaanku yahay mid togan:",
        secondaryList: [
          "Isku day inaad sameysato ballan uurka kahor 1-2 toddobaad gudahood",
          "Bilow qaadashada fiitamiinka uurka (uma baahnid warqad dhakhtar)",
          "Codso caymiska caafimaadka haddii aad u baahan tahay",
        ],
        tertiaryTitle: "Codsiga Caymiska Caafimaadka Inta Uurka Lagu Jiro",
        tertiaryLink: "#section-4-3",
        closing: [
          "Si aad u codsato caymiska caafimaadka, waxaa laga yaabaa inaad u baahato warqad caddaynta uurka oo ka timaada bixiyaha daryeelka caafimaadka.",
          "Talo badbaadinta kharashka: Waaxda Caafimaadka Dadweynaha ee Two Rivers ayaa ku siin karta baaritaanka uurka iyo warqadda caddaynta iyada oo aan booqasho rug caafimaad loo baahnayn.",
        ],
      },
      {
        title: "Haddii Aad U Baahan Tahay Daryeel Degdeg Ah",
        subtitle: "Daryeelka degdegga ah ayaa ugu horreeya",
        intro: [
          "Haddii aad leedahay xanuun daran, dhiig-bax badan, qandho, ama aad dareemayso ammaan-darro, hel daryeel degdeg ah.",
          "Tag qolka xaaladaha degdegga ah kasta.",
        ],
        listTitle: "Laguma celin karo sababtoo ah:",
        listItems: [
          "Xaaladda caymiska",
          "Xaaladda socdaalka",
          "Awoodda bixinta",
        ],
      },
      {
        title: "Ma U Baahan Tahay Caawimaad Dheeraad Ah Hadda?",
        subtitle: "Taageero dheeraad ah inta lagu jiro uurka",
        listTitle:
          "Caawimaad ayaa la heli karaa haddii aad ka walwalsan tahay:",
        listItems: [
          "Cuntada",
          "Guriyeynta",
          "Gaadiidka",
          "Badbaadada",
          "Walaaca socdaalka",
        ],
        secondaryTitle: "Ma aha inaad keligaa xalliso tan.",
        tertiaryTitle:
          "Taageero ayaa la heli karaa. Fadlan eeg hagaha kheyraadka ee agabka maxalliga ah.",
        tertiaryLink: "/resources",
      },
      {
        title: "Ikhtiyaarkaaga uurka",
        subtitle: "Baro barbaarinta, korsashada, iyo ikhtiyaarka uurka",
        intro: [
          "Dadku waxay doortaan waddooyin kala duwan inta lagu jiro uurka.",
          "Dadka qaar:",
        ],
        listItems: [
          "Waxay sii wadaan uurka oo ay barbaariyaan",
          "Waxay sameeyaan qorshe korsasho",
          "Waxay doortaan inay joojiyaan uurka",
        ],
        secondaryTitle: "Korsashada",
        secondaryIntro: "Korsashadu waa ikhtiyaar sharci ah. Waad:",
        secondaryList: [
          "Doorata korsasho furan, nus-furan, ama xiran (halka waalidiinta dhalay ay si buuxda ugu lug leeyihiin, qayb ahaan ugu lug leeyihiin, ama aysan ku lug lahayn)",
          "Doorata qoyska ilmahaaga",
          "Helaa caawimaad kharashaadka la xiriira uurka qaarkood",
          "Beddelaa maskaxdaada ilaa korsashada si sharci ah loo dhameystiro",
        ],
        tertiaryTitle: "Ilmo iska soo rididda",
        tertiaryIntro:
          "Sharciga Nebraska wuxuu xaddidaa ilmo iska soo rididda ka dib 12 toddobaad oo uur ah, marka laga reebo dhowr xaaladood.",
        tertiaryList: [
          "La xiriir Planned Parenthood wixii talo bixin ah oo ku saabsan ilmo iska soo rididda",
        ],
      },
      {
        title: "Sababta Daryeelka Uurka Hore U Yahay Muhiim",
        subtitle: "Sida daryeelka hore uu kuu caawiyo adiga iyo ilmahaaga",
        intro: "Arkidda bixiyaha daryeelka uurka goor hore waxay caawisaa:",
        listItems: [
          "Ilaalinta caafimaadkaaga",
          "Ilaalinta caafimaadka ilmahaaga",
          "Helitaanka dhibaatooyinka goor hore",
          "Kugu xirida caymiska, caawimaadda cuntada, iyo taageero kale",
        ],
        closing:
          "Isku day inaad bilowdo daryeelka uurka sida ugu dhakhsaha badan ee aad awooddo.\nQaado fiitamiinka uurka maalin kasta haddii ay suurtagal tahay.",
      },
    ],
  },
  section2: {
    title: "Daryeelka Dhalmada Kahor: Helitaanka & Sida ay u Shaqeyso",
    description: [
      "Qaybtani waxay sharraxaysaa sida daryeelka dhalmada kahor u shaqeeyo.",
      "Waxaad baran doontaa:",
    ],
    descriptionListItems: [
      "Halka laga helo daryeelka dhalmada kahor",
      "Waxa badanaa ka dhaca booqashooyinka",
      "Sida daryeelku u eg yahay qaybaha kala duwan ee uurka",
    ],
    descriptionClosing: [
      "Qaybtani waxay sidoo kale ku jirtaa macluumaad ku saabsan daryeelka dhalmada kadib.",
      "Waxaad sidoo kale heli doontaa hagitaan ku saabsan sida wax loo weydiiyo, loo helo caawimaad fahamka macluumaadka, iyo in la hadlo inta lagu jiro daryeelka. Waxaad tahay qayb ka mid ah kooxda daryeelka.",
      "Hagaha Kheyraadka ayaa kaa caawin kara inaad hesho rugaha caafimaadka iyo adeegyada daryeelka dhalmada kahor iyo kadib.",
      "Qaybtan waxaa loo isticmaali karaa tixraac ahaan inta lagu jiro uurka iyo dhalashada kadib.",
    ],
    accordion: [
      {
        title: "U Doodista Naftaada Daryeelka Caafimaadka",
        subtitle:
          "Waxaad xuquuq u leedahay inta lagu jiro daryeelka caafimaadka.",
        listTitle: "Waxaad xaq u leedahay inaad:",
        listItems: [
          "Weydiiso su'aalo",
          "Weydiiso turjumaan",
          "Qof wadato",
          "Weydiiso nuqulada diiwaankaaga",
          "Beddesho bixiyeyaasha",
        ],
        secondaryTitle: "Waad dhihi kartaa:",
        secondaryList: [
          "“Ma sharxi kartaa taas mar kale?”",
          "“Waxaan u baahanahay turjumaan.”",
          "“Wax baa qaldan oo aan dareemayaa.”",
        ],
      },
      {
        title: "Doorashada Bixiyaha Daryeelka Dhalmada Kahor",
        subtitle: "Halka aad ka heli doonto daryeel oo aad ku umuli doonto",
        intro: [
          "Dhammaan rugaha caafimaadku ma bixiyaan daryeelka dhalmada kahor.",
          "Dhammaan isbitaalladu ma umuliyaan dhallaanka.",
          "Xaalad degdeg ah, tag qolka xaaladaha degdegga ah.",
          "Way ku caawinaysaa in goor hore la weydiiyo halka aad ku umuli doonto.",
        ],
        listTitle: "Markaad wacayso rug caafimaad, waxaad weydiin kartaa:",
        listItems: [
          "Ma bixisaan daryeelka dhalmada kahor?",
          "Ma umulisaan dhallaanka?",
          "Halkee ayaan ku umuli lahaa?",
          "Isbitaal noocee ah ayaan aadaa?",
          "Maxaa dhacaya haddii fooshu bilaabato habeenkii?",
          "Yaan wacaa?",
          "Ma leedihiin turjumaan?",
        ],
        secondaryTitle: "Meelaha Miyiga ah",
        secondaryList: [
          "Dhakhaatiirta qoyska ayaa badanaa bixiya daryeelka uurka",
          "Dadka qaar ayaa ku umula isbitaal ka duwan halka ay ka helaan daryeelka dhalmada kahor",
          "Uurka khatarta sare leh waxaa laga yaabaa inay ku lug yeeshaan takhasusleyaal",
        ],
        closing:
          "Ogaanshaha qorshaha ka hor waxay caawisaa dhimista walbahaarka iyo layaabka.\nWaa caadi in su'aalahan la is weydiiyo wax ka badan hal jeer.",
      },
      {
        title: "Goobaha Daryeelka Dhalmada Kahor",
        subtitle: "Halka laga helo daryeel dhow",
        intro: "Eeg hagaha kheyraadka wixii macluumaad dheeraad ah.",
        introLink: "/resources",
        listTitle: "Kearney",
        listItems: [
          "CHI Health Good Samaritan (Xarunta Umusha) — Foosha & dhalmada + daryeelka umusha kadib (xarunta umusha gobolka).",
          "Kearney Regional Medical Center (Uurka & Ilmaha / Umusha + NICU) — Foosha & dhalmada oo leh heerka II NICU iyo taageerada naasnuujinta.",
          "Family Practice Associates (Kearney) — Daryeelka dhalmada kahor oo lala yeesho dhakhaatiir ku umulisa isbitaallada Kearney.",
          "Contemporary Obstetrics & Gynecology (Kearney) — Rugta OB/GYN oo bixisa daryeelka uurka iyo adeegyada caafimaadka haweenka.",
        ],
        secondaryTitle: "Lexington",
        secondaryList: [
          "Lexington Regional Health Center (Dhalmada / Foosha & Dhalmada) — Daryeelka dhalmada kahor ilaa dhalmada iyo taageerada umusha kadib.",
          "Lexington Women’s Care — Rugta OB/GYN oo leh daryeelka dhalmada ee caadiga ah iyo midka khatarta sare leh.",
          "Plum Creek Medical Group (Lexington) — Rugta daawada qoyska; waxay bixin kartaa gudbinta/isku-dubbaridka dhalmada kahor.",
        ],
        tertiaryTitle: "Cozad, Gothenburg & North Platte",
        tertiaryList: [
          "Cozad Community Medical Clinic — Rugta daawada qoyska oo ay ku jirto daryeelka dhalmada kahor.",
          "Gothenburg Health (Caafimaadka Haweenka / Adeegyada OB) — Daryeelka dhalmada kahor, foosha & dhalmada, umusha kadib + taageerada naasnuujinta.",
          "Great Plains Health (Uurka & Dhalmada / Adeegyada Umusha) — Daryeelka dhalmada kahor iyo adeegyada dhalmada ee isbitaalka ee North Platte.",
          "North Platte OBGYN, PC — Rugta OB/GYN oo bixisa daryeelka uurka iyo dhalmada (caafimaadka haweenka).",
        ],
        quaternaryTitle: "Daryeelka Uurka Khatarta Sare leh",
        quaternaryList: [
          "CHI Health Maternal–Fetal Medicine (TeleMFM) — Taageero takhasus leh oo loogu talagalay uurka khatarta sare leh (badanaa iyada oo loo marayo telehealth).",
        ],
      },
      {
        title: "Sida Caadiga ah Daryeelka Dhalmada Kahor U Eg Yahay",
        subtitle: "Waxa dhaca inta lagu jiro daryeelka uurka",
        intro: [
          "Daryeelka dhalmada kahor macnaheedu waa baaritaanno joogto ah inta lagu jiro uurka si lagaaga caawiyo ilaalinta caafimaadkaaga iyo kan ilmahaaga.",
          "Uma baahnid inaad fahanto erayada caafimaadka si aad u hesho daryeel wanaagsan.",
          "Qaybtani waxay sharraxaysaa waxa badanaa dhaca, si aad u ogaato waxa la filayo — iyo waxa la weydiisto.",
        ],
        subAccordion: [
          {
            title: "Bilaabidda Daryeelka & Booqashooyinka Hore",
            intro:
              "Dadka intooda badan waxay leeyihiin booqashadooda koowaad ee dhalmada kahor bilaha ugu horreeya uurka.",
            listTitle:
              "Booqashada koowaad, rugta caafimaadku waxay laga yaabaa inay:",
            listItems: [
              "Xaqiijiso uurka",
              "Qiyaasto taariikhda dhalmadaada",
              "Wax ka weydiiso caafimaadkaaga, uurkaagii hore, iyo daawooyinka",
              "Hubiso cadaadiska dhiiggaaga iyo miisaankaaga",
              "Dalbato baaritaannada dhiigga iyo kaadida",
              "Mararka qaarkood sameyso ultrasound hore",
            ],
            secondaryTitle: "Waad weydiin kartaa:",
            secondaryList: [
              "“Waa maxay taariikhda dhalmadaydu?”",
              "“Goorma ayaan soo laabtaa?”",
              "“Yaan wacaa haddii aan su'aalo qabo?”",
            ],
          },
          {
            title: "Booqashooyinka Joogtada ah & Baaritaannada Caadiga ah",
            intro:
              "Booqashada koowaad kadib, dadka intooda badan waxay leeyihiin baaritaanno joogto ah inta lagu jiro uurka.",
            listTitle: "Booqashooyin badan, badanaa waxay:",
            listItems: [
              "Hubiyaan cadaadiska dhiiggaaga",
              "Hubiyaan miisaankaaga",
              "Hubiyaan kaadidaada",
              "Dhageystaan garaaca wadnaha ilmaha",
              "Weydiiyaan sida aad dareemayso",
              "Siiyaan waqti su'aalaha",
            ],
            closing:
              "Haddii aan weligeed wax la hubin, waa caadi in la weydiiyo sababta.",
          },
          {
            title: "Baaritaannada, Ultrasounds & Tallaalada",
            intro:
              "Inta lagu jiro uurka, baaritaanno qaarkood ayaa badanaa la bixiyaa si loo hubiyo caafimaadkaaga iyo koritaanka ilmaha.",
            listTitle: "Dad badan ayaa leh:",
            listItems: [
              "Baaritaannada dhiigga goor hore uurka",
              "Ultrasound bartamaha uurka (agagaarka 20 toddobaad)",
              "Baaritaanka sokorowga goor dambe uurka",
              "Tallaalada adiga iyo ilmahaaga ilaaliya",
            ],
            secondaryTitle:
              "Baaritaannada qaarkood waa ikhtiyaari. Waad weydiin kartaa:",
            secondaryList: [
              "“Muxuu yahay baaritaankani?”",
              "“Ma inaan maanta go'aan gaaraa?”",
            ],
          },
          {
            title: "Isbeddelada Jirkaaga Waa Caadi",
            intro: "Uurku wuxuu keenaa isbeddelo badan oo jirka ah.",
            listTitle: "Isbeddelada caadiga ah waxaa ka mid ah:",
            listItems: [
              "Daal",
              "Lallaabbo ama matag",
              "Xanuunka jirka",
              "Laabjeex",
              "Barar",
              "Isbeddelada niyadda",
            ],
            closing:
              "Ma tihid mid daciif ah ama sameynaya wax qaldan.\nMar walba u sheeg bixiyahaaga sida aad dareemayso.",
          },
          {
            title: "Goorta La Helo Caawimaad Degdeg Ah",
            listTitle: "Ha sugin ballantaada xigta haddii aad leedahay:",
            listItems: [
              "Dhiig-bax badan",
              "Xanuun caloosha oo xooggan",
              "Qandho",
              "Madax-xanuun daran ama dhibaatooyin aragga",
              "Barar degdeg ah oo wejiga ama gacmaha",
              "Neefsashada oo dhibta",
              "Dheecaan ka soo baxaya siilka",
              "Ilmaha oo u dhaqaaqaya wax aad uga yar sidii hore (goor dambe uurka)",
            ],
            secondaryTitle: "Waxaad aadi kartaa:",
            secondaryList: [
              "Rugtaada caafimaadka",
              "Daryeelka degdegga ah",
              "Qolka xaaladaha degdegga ah kasta",
            ],
            closing: "Laguma celin karo caymis la'aan.",
          },
          {
            title: "Sida Aad U Caawiso Daryeelkaaga",
            listTitle: "Waxaad daryeelkaaga ku caawisaa adigoo:",
            listItems: [
              "Aadaya booqashooyinka marka aad awooddo",
              "Weydiinaya su'aalo",
              "Hadlaya haddii wax qaldan la dareemo",
              "Weydiisanaya caawimaad marka ay arrimuhu wareer yihiin",
            ],
            closing:
              "Daryeelka dhalmada kahor ee wanaagsan waa wax aad mudan tahay.",
          },
        ],
      },
    ],
  },
  section3: {
    title: "Daryeelka Uurka Maalinlaha ah",
    description: [
      "Qaybtani waxay diiradda saartaa waxa dad badan ay sameeyaan maalin kasta si ay u taageeraan caafimaadkooda inta lagu jiro uurka.",
      "Waxay daboolaysaa cuntada, nasashada, dhaqdhaqaaqa, daawooyinka, walbahaarka, iyo isbeddelada caadiga ah ee jirkaaga. Doorashooyinkan maalinlaha ah waxay taageeraan caafimaadkaaga iyo koritaanka ilmahaaga. Uma baahnid inaad wax walba si sax ah u sameyso. Tallaabooyin yaryar oo joogto ah ayaa weli caawinaya adiga iyo ilmahaaga.",
      "Hagaha Kheyraadka waxaa ku jira barnaamijyo iyo taageerooyin bulsho oo la xiriira fiyoobida uurka oo laga yaabo inaad waxtar u hesho. Waxaad ku soo laaban kartaa qaybtan wakhti kasta oo aad qabtid su'aalo maalinle ah ama aad u baahan tahay kalsooni.",
    ],
    accordion: [
      {
        title: "Fayo-qabka Uurka & Daryeelka Maalinlaha ah",
        subtitle:
          "Daryeel maalinle ah oo fudud iyo goorta la weydiisto caawimaad",
        intro: [
          "Uurku wuxuu shaqo dheeraad ah saaraa jirkaaga.",
          "Doorashooyinka maalinlaha ah ee yaryar waxay caawin karaan taageerada caafimaadkaaga iyo caafimaadka ilmahaaga.",
          "Uma baahnid inaad wax walba si qumman u sameyso.",
          "Sameynta qaar ka mid ah waxyaabahan ayaa mar hore caawinaya.",
        ],
        subAccordion: [
          {
            title: "Cuntada & Nafaqada",
            intro:
              "Cunista si joogto ah waxay caawisaa jirkaaga iyo in ilmahaagu koro.",
            listTitle: "Isku day inaad:",
            listItems: [
              "Cunto cuntooyinka ama cuntooyinka fudud maalinta oo dhan",
              "Ku darto miraha, khudradda, badarka, iyo borotiinka marka ay suurtagal tahay",
              "Cabto biyo badanaa",
            ],
            secondaryTitle: "Haddii cuntadu xadidan tahay:",
            secondaryList: [
              "Cun waxaad awooddo",
              "Weydii wax ku saabsan WIC ama taageerada cuntada",
              "U sheeg rugtaada caafimaadka haddii aad ka boodayso cuntooyinka",
            ],
            tertiaryTitle: "Waad weydiin kartaa:",
            tertiaryList: [
              "“Ma jirtaa taageero cunto oo aan heli karo inta aan uurka leeyahay?”",
              "“Cuntooyinkee caawiya lallaabbada ama laabjeexa?”",
            ],
          },
          {
            title: "Fiitamiinada Uurka",
            intro:
              "Fiitamiinada uurka waxay caawiyaan buuxinta daldaloolada nafaqada.",
            listTitle: "Isku day inaad:",
            listItems: [
              "Qaadato hal fiitamiin uurka maalin kasta",
              "Kula qaadato cunto haddii ay calooshaada xanuujiso",
              "Waxaad weydiisan kartaa fiitamiinnada uurka ee aan lahayn gelatin ama xalaal ah",
            ],
            secondaryTitle: "Waad dhihi kartaa:",
            secondaryList: ["“Fiitaminkani wuxuu i dareensiiyaa jirro.”"],
            closing: [
              "Haddii aad illowdo mararka qaarkood, waa caadi.",
              "Haddii fiitamiinadu ku xanuujiyaan, u sheeg bixiyahaaga.",
              "Ikhtiyaarka kale ayaa la heli karaa.",
            ],
          },
          {
            title: "Nasashada, Shaqada, iyo Nolosha Maalinlaha ah",
            intro:
              "Jirkaagu wuxuu u baahan yahay nasasho dheeraad ah inta lagu jiro uurka.",
            listTitle: "Haddii ay suurtagal tahay:",
            listItems: [
              "Naso marka aad daalan tahay",
              "Fariiso ama jiifso inta lagu jiro nasashada",
              "Ka fogow qaadista alaabta aadka u culus",
            ],
            secondaryIntro:
              "Haddii aad uur leedahay inta lagu jiro bisha Soonqaad (Ramadan), waxaa laga yaabaa inaad qabto su'aalo ku saabsan soonka. Islaamku wuxuu ogol yahay dabacsanaan xilliga uurka haddii soonku saameyn ku yeelan karo caafimaadkaaga ama caafimaadka ilmahaaga.",
            secondaryTitle: "Haddii shaqadu ku adag tahay jirkaaga:",
            secondaryList: [
              "U sheeg bixiyahaaga waxa shaqadaadu u baahan tahay",
              "Weydii haddii qoraal shaqo ama hagaajin shaqo loo baahan yahay",
            ],
            tertiaryTitle: "Waad dhihi kartaa:",
            tertiaryList: [
              "“Shaqadaydu waa mid jireed aad u badan.”",
              "“Waan daalanahay mar walba.”",
            ],
          },
          {
            title: "Dhaqdhaqaaqa & Jimicsiga",
            intro:
              "Dhaqdhaqaaqa fudud badanaa waa caafimaad inta lagu jiro uurka.",
            listTitle: "Dad badan ayaa ka faa'iideysta:",
            listItems: [
              "Socodka",
              "Fidin",
              "Dhaqdhaqaaqa maalinlaha ah ee fudud",
            ],
            secondaryTitle: "Jooji oo hel caawimaad haddii aad dareento:",
            secondaryList: [
              "Wareer",
              "Xanuun laabta ah",
              "Xanuun xooggan",
              "Dhiig-bax",
            ],
            tertiaryTitle: "Waad weydiin kartaa:",
            tertiaryList: ["“Noocee dhaqdhaqaaq ah ayaa ii ammaan ah?”"],
          },
          {
            title: "Walbahaarka, Niyadda & Caafimaadka Dareenka",
            intro: "Uurku wuxuu keeni karaa:",
            listItems: [
              "Walbahaar",
              "Walwal",
              "Murugo",
              "Xanaaq",
              "Dhibaato hurdo",
            ],
            secondaryTitle:
              "Dareemadan waa kuwo caadi ah — oo caawimaad ayaa la heli karaa.",
            tertiaryTitle: "U sheeg bixiyahaaga haddii aad dareento:",
            tertiaryList: [
              "Culeys badan",
              "Murugo badan",
              "Walaac badan",
              "Ammaan-darro",
            ],
            quaternaryTitle: "Waad dhihi kartaa:",
            quaternaryList: [
              "“Ma dareemayo sidii naftayda.”",
              "“Waxaan ku dhibtoonayaa inaan la qabsado.”",
            ],
            closing: "Waxaad mudan tahay taageero inta lagu jiro uurka.",
          },
          {
            title: "Caabuqyada & Joogitaanka Caafimaad",
            listTitle: "Si aad iskaga caawiso ilaalinta naftaada:",
            listItems: [
              "Dhaq gacmahaaga badanaa",
              "Ka fogow dadka aadka u xanuunsan marka ay suurtagal tahay",
              "Tag ballamaha oo hel tallaalada lagu taliyay",
            ],
            secondaryTitle: "Wac ama raadi daryeel haddii aad leedahay:",
            secondaryList: [
              "Qandho",
              "Gubasho kaadida",
              "Xanuun caloosha oo xooggan",
              "Dhiig-bax siilka",
            ],
          },
        ],
      },
    ],
  },
  section4: {
    title: "Bixinta Daryeelka, Waraaqaha & Taageerada",
    description: [
      "Qaybtani waxay kaa caawinaysaa caymiska, waraaqaha, iyo taageerada dheeraadka ah.",
      "Dad badan ayaa u baahan caawimaad bixinta daryeelka, fahamka foomamka, ama helitaanka cuntada, guriyeynta, gaadiidka, ama taageerada badbaadada.",
      "Taasi waa caadi — oo caawimaad ayaa la heli karaa.",
      "Waxaad eegi kartaa hal qayb markiiba.",
      "Uma baahnid inaad fahanto wax walba si aad u bilowdo.",
    ],
    accordion: [
      {
        title: "Helitaanka Caymiska Caafimaadka Inta Uurka Lagu Jiro",
        subtitle: "Sida loo helo caymis iyo waxa uu caawiyo bixinta",
        intro: [
          "Waxaa jira barnaamijyo kaa caawin kara bixinta daryeelka caafimaadka iyo cuntada inta lagu jiro uurka.",
          "Uma baahnid inaad fahanto dhammaan barnaamijyada si aad u bilowdo. Caawimaad ayaa la heli karaa xitaa haddii xaaladdaada socdaalka ay adag tahay.",
          "Barnaamijyo kala duwan ayaa siyaabo kala duwan u caawiya:",
        ],
        listItems: [
          "Qaar waxay bixiyaan caymiska caafimaadka",
          "Qaar waxay bixiyaan daryeelka uurka oo kaliya",
          "Qaar waxay ku caawiyaan cuntada iyo nafaqada",
        ],
        closing:
          "Waxaad maqli kartaa barnaamijyadan loogu yeero Medicaid, CHIP, ama WIC. Faahfaahin dheeraad ah ayaa laga heli karaa hoos.",
      },
      {
        title: "Taageero Xaaladaha Gaarka ah",
        subtitle: "Caawimaad dheeraad ah waa caadi waana la heli karaa.",
        intro:
          "Uurku wuxuu keeni karaa walwal cusub oo ku saabsan lacagta, guriyeynta, cuntada, badbaadada, ama waraaqaha.",
        listTitle: "Caawimaad ayaa la heli karaa:",
        listItems: [
          "Walaaca socdaalka",
          "Taageerada cuntada",
          "Caawimaadda guriyeynta",
          "Gaadiidka",
          "Walaaca badbaadada",
        ],
        secondaryTitle: "Muhiim in la ogaado",
        secondaryList: [
          "Rugaha caafimaadka iyo isbitaalladu ma soo sheegaan xaaladda socdaalka",
          "Weydiisashada caawimaad saameyn kuma yeelan doonto daryeelkaaga caafimaadka",
          "Waad weydiisan kartaa caawimaad xitaa haddii aadan hubin waxaad u baahan tahay",
        ],
        closing: "Ma aha inaad keligaa xalliso tan.",
      },
      {
        title: "Caawimaad Dheeraad Ah Caymiska & Waraaqaha",
        subtitle:
          "Barnaamijyada, codsiyada, dukumentiyada, iyo tallaabooyinka xiga",
        secondaryTitle: "Fahamka Medicaid, CHIP & WIC",
        secondaryIntro:
          "Barnaamijyo kala duwan ayaa siyaabo kala duwan u caawiya:",
        secondaryList: [
          "Medicaid – Caymiska caafimaadka oo caawiya bixinta daryeelka caafimaadka",
          "CHIP (Barnaamijka Caymiska Caafimaadka Carruurta) – Caymiska caafimaadka ee carruurta iyo dadka uurka leh qaarkood",
          "WIC (Haweenka, Dhallaanka, iyo Carruurta) – Cuntada, nafaqada, iyo taageerada naasnuujinta",
        ],
        closing:
          "Uma baahnid inaad ogaato barnaamijka kugu habboon ka hor inta aadan codsan.",
      },
      {
        title: "Sida Loo Codsado Caymiska Caafimaadka",
        subtitle: "Siyaabaha loo codsado loona helo caawimaad",
        intro: [
          "Siyaabaha loo codsado loona helo caawimaad",
          "Waxaa jira hal codsi oo muhiim ah Medicaid iyo CHIP ee Nebraska.",
          "Uma baahnid inaad ogaato barnaamijka aad u qalanto ka hor inta aadan codsan.",
          "Gobolka ayaa dib u eegi doona macluumaadkaaga oo go'aamin doona waxa ku habboon xaaladdaada.",
          "Dad badan ayaa u baahan caawimaad codsiga — taasi waa caadi.",
          "Waxaad u codsan kartaa siyaabo kala duwan:",
        ],
        listItems: ["Online", "Taleefan", "Shaqsi ahaan"],
        secondaryIntro: [
          "Waxaad dooran kartaa ikhtiyaarka kuugu fiican.",
          "Codso xitaa haddii dukumentiyo maqan yihiin.",
          "Shaqaalaha ayaa kuu sheegi kara waxa la keeno hadhow.",
        ],
        tertiaryTitle: "Waad dhihi kartaa:",
        tertiaryIntro:
          "“Waxaan rabaa inaan codsado caymiska caafimaadka uurka. Ma i caawin kartaa?”",
        subAccordion: [
          {
            title: "Suuqa Caymiska Caafimaadka",
            intro:
              "Dadka uurka leh qaarkood waxay isticmaalaan caymiska iyada oo loo marayo Suuqa Caymiska Caafimaadka (Healthcare.gov).",
            listItems: [
              "Ay iskood u shaqeystaan",
              "Aysan u qalmin Medicaid ama CHIP",
              "Ay rabaan caymis sii socda kadib marka ilmahaagu dhasho",
            ],
            closing:
              "Qorshayaasha Suuqa waxaa laga yaabaa inay leeyihiin kharashyo bille ah, laakiin dad badan ayaa u qalma caawimaad dhaqaale.\nCaawimaad ayaa la heli karaa si loo barbardhigo qorshayaasha oo loo codsado.",
          },
          {
            title: "Dukumentiyada laga yaabo in lagu weydiiyo",
            intro: [
              "Keligaa ma tihid. Qaad tallaabooyin yaryar oo cad.",
              "Dukumentiyada ayaa badanaa la codsadaa kadib marka aad codsato.",
              "Waa caadi haddii aadan wax walba haysan.",
            ],
            listTitle: "Warqadda Caddaynta Uurka",
            listItems: [
              "Waxay xaqiijinaysaa inaad uur leedahay.",
              "Waxaad tan ka heli kartaa: Waaxda Caafimaadka Dadweynaha ee Two Rivers ama ballanta rugta caafimaadka",
              "Talo badbaadinta kharashka: Caafimaadka Dadweynaha ayaa ku siin karta warqaddan iyada oo aan booqasho rug caafimaad loo baahnayn.",
              "Waad dhihi kartaa: “Waxaan u baahanahay warqad caddaynta uurka si aan u codsado caymiska caafimaadka.”",
            ],
            secondaryTitle: "Foom Aqoonsi",
            secondaryIntro: "Dukumenti kasta oo magacaaga wata, sida:",
            secondaryList: [
              "Shatiga darawalnimada ama aqoonsiga gobolka",
              "Baasaboor (waddan kasta)",
              "Aqoonsiga dugsiga, shaqada, ama qunsuliyadda",
            ],
            tertiaryTitle: "Caddaynta Cinwaanka",
            tertiaryIntro:
              "Waxay muujinaysaa halka aad hadda ku nooshahay, sida:",
            tertiaryList: [
              "Boosto magacaaga wata",
              "Heshiiska kirada ama biilka adeegga",
              "Qoraal ka yimid qofka aad la nooshahay ama hoyga",
            ],
            quaternaryTitle: "Caddaynta Dakhliga (haddii la heli karo)",
            quaternaryList: [
              "Tusaalooyinka: Jeegaga mushaarka ama warqadda loo shaqeeyaha",
              "Diiwaannada bangiga",
              "Bayaanka dakhli la'aanta",
            ],
            closing: "Codso xitaa haddii dukumentiyo maqan yihiin.",
          },
          {
            title: "Inta Aad Sugayso Caymiska",
            intro: "Laga yaabo inaadan helin kaarka caymiska isla markiiba.",
            listTitle: "Muhiim in la ogaado:",
            listItems: [
              "Ansixinta ayaa dhici karta kadib marka daryeelku bilowdo",
              "Medicaid ayaa bixin kara daryeelka ilaa 3 bilood ka hor ansixinta (Medicaid dib u noqosho leh)",
            ],
            secondaryTitle: "Sababtoo ah tan:",
            secondaryList: [
              "Waxaa laga yaabaa in lagu weydiiyo inaad lacag caddaan ah bixiso inta aad sugayso",
              "U sheeg rugaha caafimaadka inaad codsatay Medicaid",
              "Hayso rasiidada haddii caymiska la ansixiyo hadhow",
            ],
            tertiaryTitle: "Haddii aad u baahan tahay daryeel degdeg ah:",
            tertiaryList: [
              "Tag qolka xaaladaha degdegga ah kasta",
              "Laguma celin karo caymis la'aan",
            ],
            closing: "Caafimaadkaaga iyo badbaadadaada ayaa ugu horreeya.",
          },
          {
            title: "Macluumaad Muhiim Ah Oo La Hayo",
            intro:
              "Haddii aad awooddo, macluumaadkan kula hay — gaar ahaan haddii aad guurto ama aad beddesho rugaha caafimaadka.",
            listTitle: "Waraaqo muhiim ah",
            listItems: [
              "Warqadda caddaynta uurka",
              "Waraaqaha caymiska ama Medicaid",
              "Aqoonsiga, cinwaanka, iyo waraaqaha dakhliga (haddii la heli karo)",
              "Qoraallada booqashada dhalmada kahor",
              "Warbixinnada ultrasound",
              "Natiijooyinka shaybaarka",
            ],
            secondaryTitle: "Macluumaad muhiim ah oo ku saabsan uurka",
            secondaryList: [
              "Taariikhda dhalmada (ama qiyaasta ugu fiican)",
              "Dhibaatooyinka uurka",
              "Uurka hore ama dhibaatooyinka dhalmada",
            ],
            tertiaryTitle: "Daawooyinka iyo xasaasiyadaha",
            tertiaryList: [
              "Daawooyinka aad qaadato",
              "Fiitamiinada ama kaabayaasha",
              "Xasaasiyadda daawada ama cuntada",
            ],
            quaternaryTitle: "Talooyin waxtar leh",
            quaternaryList: [
              "Ka qaad sawirro waraaqaha taleefankaaga",
              "Waraaqaha ku hay hal meel",
            ],
            closing: "Weydiiso nuqulo ka hor inta aadan ka tegin ballamaha",
          },
        ],
      },
    ],
  },
};

export const sectionData = {
  en: sectionDataEn,
  es: sectionDataEs,
  so: sectionDataSo,
};
